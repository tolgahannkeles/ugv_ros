...
```
                ...
                ┌─────────▼─────────────┐
                │ diff_drive_controller │  ros2_control (ugv_control)
                └─────────┬─────────────┘
                          │ wheel velocity commands
          ┌───────────────┴────────────────┐
┌─────────▼──────────────┐     ┌───────────▼──────────────┐
│ ugv_hardware/          │     │ gz_ros2_control/         │
│ ESP32System (UART)     │     │ GazeboSimSystem          │
│   ◄──► ESP32           │     │   ◄──► Gazebo            │
└────────────────────────┘     └──────────────────────────┘
        real robot                      simulation

IMU + GPS ──► /imu/data, /gps/fix ──► ugv_localization
...
The fully autonomous depth estimation + obstacle avoidance nodes in `ugv_intelligence` (`depth_node`, `avoidance_node`) exist in code but are **disabled**. Instead, the lightweight `collision_guard` node provides **driver-assisted collision avoidance** (see below).

## Packages (`src/`)

| Package | Build type | Description |
|---|---|---|
| [ugv_description](src/ugv_description) | ament_cmake | URDF (xacro) of the tracked chassis: 3 wheels per side, only the rear sprocket is driven and the front/middle wheels follow it through the track (`<mimic>`). `ros2_control.xacro` (Gazebo / mock / ESP32 hardware), `gazebo.xacro` (sensors with noise). `rsp.launch.py`, `display.launch.py`, RViz config |
| [ugv_control](src/ugv_control) | ament_cmake | `controllers.yaml` (`diff_drive_controller`, `joint_state_broadcaster`), `twist_mux.yaml` (TwistStamped). Shared by the robot and the simulation |
| [ugv_localization](src/ugv_localization) | ament_cmake | robot_localization dual EKF + `navsat_transform` (`ekf.yaml`, `localization.launch.py`) |
| [ugv_hardware](src/ugv_hardware) | ament_cmake (C++) | ros2_control plugin `ugv_hardware/ESP32System`: ESP32 UART protocol, track commands, IMU/GPS telemetry |
| [ugv_bringup](src/ugv_bringup) | ament_python | Real robot: `robot.launch.py` and robot-specific parameter overrides |
| [ugv_gazebo](src/ugv_gazebo) | ament_cmake | Simulation: world, `gz_bridge.yaml`, `sim.launch.py`, `gps_driver.py` |
| [ugv_intelligence](src/ugv_intelligence) | ament_python | `collision_guard` (active) + ONNX depth estimation/autonomous avoidance (disabled) |
| [ugv_web](src/ugv_web) | ament_python | Web UI server and static files (control panel) |

### ugv_bringup (real robot)

`launch/robot.launch.py` starts:

- `robot_state_publisher` with the URDF (`ugv_description/rsp.launch.py`)
- `ros2_control_node` with `ugv_control/config/controllers.yaml` plus `config/controllers_real.yaml`, and spawners for `joint_state_broadcaster` and `diff_drive_controller`
- `imu_filter_madgwick` (`/imu/data_raw` → `/imu/data`) and `ugv_localization` with `config/localization_real.yaml`
...
`ugv_hardware/ESP32System` is a ros2_control `SystemInterface` that owns the UART link to the ESP32.
...
- `include/ugv_hardware/protocol.hpp`: a checksummed, state-machine-based frame protocol. `include/ugv_hardware/uart_driver.hpp`: a POSIX `termios` serial driver, **Linux-only**.
...
- Python 3, `onnxruntime`, `cv_bridge` (for `ugv_intelligence`)
- `ugv_hardware` builds on Linux only (POSIX `termios`).
...
ros2 launch ugv_bringup robot.launch.py use_mock_hardware:=true use_camera:=false
```
...
`ugv_gazebo/launch/sim.launch.py` runs the same layers as the robot. `gz_ros2_control` replaces the ESP32 hardware plugin, and `ros_gz_bridge` provides the sensors.
...
- **Sensor contract:** camera, IMU (with ENU orientation) and GPS are required; LiDAR is optional (`use_lidar:=true`). Sensors have Gaussian noise (`ugv_description/urdf/gazebo.xacro`).
...
ros2 launch ugv_gazebo sim.launch.py                  # with the Gazebo GUI
ros2 launch ugv_gazebo sim.launch.py headless:=true rviz:=true
ros2 launch ugv_description display.launch.py         # model only, in RViz
```
...
~/.venvs/cad/bin/python tools/parkur_to_gazebo.py teknofest_parkur/20260604_PRK26-A-001-001-00.STEP   src/ugv_gazebo/models/teknofest_ika_parkur
```
