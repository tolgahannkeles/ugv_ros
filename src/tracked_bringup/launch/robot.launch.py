...
Simülasyonla (tracked_gazebo/sim.launch.py) aynı katmanlar kullanılır; fark sadece
ros2_control donanım plugin'i ve gerçek robota özel parametre dosyalarıdır:
  ...
  -> diff_drive_controller -> tracked_hardware/ESP32System -> UART -> ESP32

  ESP32 -> /imu/data_raw -> Madgwick -> /imu/data ┐
  ESP32 -> /gps/fix ──────────────────────────────┴-> tracked_localization (EKF)

Kullanım:
  ros2 launch tracked_bringup robot.launch.py
  ros2 launch tracked_bringup robot.launch.py use_mock_hardware:=true use_camera:=false  # PC'de test
"""
import os
