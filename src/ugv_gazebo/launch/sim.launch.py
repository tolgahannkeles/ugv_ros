...
Kullanım:
  ros2 launch ugv_gazebo sim.launch.py                       # TEKNOFEST parkuru
  ros2 launch ugv_gazebo sim.launch.py headless:=true rviz:=true use_lidar:=true
  ros2 launch ugv_gazebo sim.launch.py world:=$(ros2 pkg prefix ugv_gazebo)/share/ugv_gazebo/worlds/test_arena.sdf x:=0 y:=0
"""
import os
...
def generate_launch_description():
    pkg_gazebo = get_package_share_directory('ugv_gazebo')
    ...
    spawn_robot = Node(
        package='ros_gz_sim',
        executable='create',
        arguments=['-topic', 'robot_description', '-name', 'ugv',
